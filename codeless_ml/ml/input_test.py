...
class InputTest(unittest.TestCase):
  ...
  def testTfRecordDataset(self):
    ...
    tf_record_file = os.path.join(os.environ["TEST_TMPDIR"], "test.tfrecord")
    ...
    def _mutate_map_entry(x):
      x["str_feat"] = tf.strings.substr(input=x["str_feat"], pos=0, len=3)
    ...
    for feature_map in dataset:
      np.testing.assert_array_equal(feature_map["str_feat"].numpy(),
